...
$columns = array(
    ...
    $sql = "SELECT id,code,name FROM product_other WHERE status=1";
    ...
    while ($row = mysqli_fetch_array($query)) {  
        $nestedData = array();
        $nestedData[] = $no;
        $nestedData[] = '<a data-toggle="modal" data-target="#otherProductDetailsModal" data-vendor='. $row['id'] .' class="waves-effect actionBtn proDetails">'.$row["code"].'</a>';
        ...
        $nestedData[] = '<button type="button" data-toggle="modal" data-target="#otherProductUpModal" data-vendor='. $row['id'] .' class="btn btn-primary waves-effect actionBtn upModelBtn" title="Edit"><i class="material-icons">edit</i></button>&nbsp;&nbsp;<button onclick="delOtherProduct('. $row['id'] .')" type="button" class="btn btn-danger waves-effect actionBtn" title="Delete"><i class="material-icons">delete</i></button>';
