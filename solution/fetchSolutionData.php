...
$columns = array(
    ...
    $sql = "SELECT id,code,name FROM product_solution WHERE status=1";
    ...
    while ($row = mysqli_fetch_array($query)) {  
        ...
        $nestedData[] = '<a data-toggle="modal" data-target="#solutionDetailsModal" data-vendor='. $row['id'] .' class="waves-effect actionBtn proDetails">'.$row["code"].'</a>';
        ...
        $nestedData[] = '<button type="button" data-toggle="modal" data-target="#solutionUpModal" data-vendor='. $row['id'] .' class="btn btn-primary waves-effect actionBtn upModelBtn" title="Edit"><i class="material-icons">edit</i></button>&nbsp;&nbsp;<button onclick="delSolution('. $row['id'] .')" type="button" class="btn btn-danger waves-effect actionBtn" title="Delete"><i class="material-icons">delete</i></button>';
